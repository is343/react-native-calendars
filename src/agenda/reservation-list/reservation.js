...
class ReservationListItem extends Component {
  ...
  renderDate(
    ...
    const today = dateutils.sameDate(date, XDate())
      ...
      : undefined;
    const holiday =
      isHoliday || dayOfTheWeek === 0 ? this.styles.holiday : undefined;
      ...
      return (
        <View style={this.styles.day}>
          <Text
            ...
            style={[this.styles.dayNum, saturday, holiday, today]}
          >
            {date.getDate()}
          </Text>
          <Text
            allowFontScaling={false}
            style={[this.styles.dayText, saturday, holiday, today]}
          >
  ...
  render() {
    ...
    if (isHoliday) {
      if (typeof this.props.renderHoliday === "function") {
        holidayContent = this.props.renderHoliday(
          this.props.holidays[dateString]
        );
      }
    }
    if (reservation) {
    ...
    if ((firstItem || !reservation) && isHoliday && holidayContent) {
      return (
        <View>
          <View
            style={[this.styles.container, date && this.styles.daySeparator]}
          >
